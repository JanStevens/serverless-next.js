...
import { Builder } from "@sls-next-n5/lambda-at-edge";
import type {
...
} from "@sls-next-n5/lambda-at-edge";
import {
  deleteOldStaticAssets,
  uploadStaticAssetsFromBuild
} from "@sls-next-n5/s3-static-assets";
import {
  createInvalidation,
...
import { removeLambdaVersions } from "@sls-next-n5/aws-lambda/dist/removeLambdaVersions";
...
class NextjsComponent extends Component {
  ...
  async deploy(
    ...
    const [
      ...
      this.load("@sls-next-n5/aws-s3"),
      this.load("@sls-next-n5/aws-cloudfront"),
      this.load("@sls-next-n5/aws-sqs"),
      this.load("@sls-next-n5/aws-lambda", "defaultEdgeLambda"),
      this.load("@sls-next-n5/aws-lambda", "apiEdgeLambda"),
      this.load("@sls-next-n5/aws-lambda", "imageEdgeLambda"),
      this.load("@sls-next-n5/aws-lambda", "regenerationLambda")
    ]);
    ...
    if (domain && subdomain) {
      const domainComponent = await this.load("@sls-next-n5/domain");
